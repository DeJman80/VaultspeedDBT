{{
	config(
		...
		alias='L_SF1_LINEITEM_LND',
		schema='RDV',
		tags=['VS_DEMO_TC', 'LND_SF1_SF1_LINEITEM_INCR', 'LND_SF1_SF1_LINEITEM_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LND_SF1_LINEITEM_HKEY" AS "LND_SF1_LINEITEM_HKEY"
		...
		FROM {{ ref('VS_DEMO_TC_STG_SF1_LINEITEM') }} "STG_SRC1"
		WHERE  "STG_SRC1"."RECORD_TYPE" = 'S'
	)
	, "MIN_LOAD_TIME" AS 
	( 
		SELECT 
			  "CHANGE_SET"."LND_SF1_LINEITEM_HKEY" AS "LND_SF1_LINEITEM_HKEY"
			...
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LND_SF1_LINEITEM_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
				"CHANGE_SET"."LOGPOSITION") AS "DUMMY"
		FROM "CHANGE_SET" "CHANGE_SET"
	)
	SELECT 
		  "MIN_LOAD_TIME"."LND_SF1_LINEITEM_HKEY" AS "LND_SF1_LINEITEM_HKEY"
	...
	LEFT OUTER JOIN {{ this }} "LND_SRC" ON  "MIN_LOAD_TIME"."LND_SF1_LINEITEM_HKEY" = "LND_SRC"."LND_SF1_LINEITEM_HKEY"
	WHERE  "LND_SRC"."LND_SF1_LINEITEM_HKEY" IS NULL AND "MIN_LOAD_TIME"."DUMMY" = 1
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."ORDER_HKEY" AS "ORDER_HKEY"
			, "CHANGE_SET"."RECORD_SOURCE" AS "RECORD_SOURCE"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LND_SF1_LINEITEM_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",
