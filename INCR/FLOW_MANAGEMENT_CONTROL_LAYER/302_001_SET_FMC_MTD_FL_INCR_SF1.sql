CREATE OR REPLACE PROCEDURE "DV_PROC"."SET_FMC_MTD_FL_INCR_SF1"(P_DAG_NAME VARCHAR2,
...
var HIST_INS = snowflake.createStatement( {sqlText: `
	...
	( 
		...
		WHERE  "FMCH_SRC"."SRC_BK" = 'SF1' AND "FMCH_SRC"."SUCCESS_FLAG" = 1 AND "FMCH_SRC"."LOAD_CYCLE_ID" < '` + P_LOAD_CYCLE_ID + `'::integer
	)
	SELECT 
...
var truncate_LWT_TGT = snowflake.createStatement( {sqlText: `
	TRUNCATE TABLE "VS_DEMO_TC_MTD"."FMC_LOADING_WINDOW_TABLE";
`} ).execute();


var LWT_INS = snowflake.createStatement( {sqlText: `
	INSERT INTO "VS_DEMO_TC_MTD"."FMC_LOADING_WINDOW_TABLE"(
		 "FMC_BEGIN_LW_TIMESTAMP"
		,"FMC_END_LW_TIMESTAMP"
	)
	SELECT 
	...
	FROM "DV_MTD"."FMC_LOADING_HISTORY" "FMCOH_SRC"
