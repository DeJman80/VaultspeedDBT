CREATE OR REPLACE PROCEDURE "DV_PROC"."SET_FMC_MTD_BV_INIT_DEMO_VS_TC"(P_DAG_NAME VARCHAR2,
...
var HIST_INS = snowflake.createStatement( {sqlText: `
	...
	( 
		...
		WHERE  "FMCH_SRC"."SRC_BK" IN('SF1') AND "FMCH_SRC"."SUCCESS_FLAG" = 1
	)
	SELECT 
...
var DV_LCI_INS = snowflake.createStatement( {sqlText: `
	...
	FROM "DV_MTD"."FMC_LOADING_HISTORY" "FMCH_LCI"
	WHERE  "FMCH_LCI"."SRC_BK" IN('SF1') AND "FMCH_LCI"."SUCCESS_FLAG" = 1
	;
`} ).execute();
