{{
	config(
		...
		alias='DIM_SUPPLIER',
		schema='BV',
		tags=['view', 'BV', 'BV_ETL_DIM_SUPPLIER_SCD2_DIM_HUB']
	)
}}
	WITH "DVO_BASE" AS 
	( 
		SELECT 
			  "HUB_SRC"."SUPPLIER_HKEY" AS "BASE_OBJECT_H_KEY"
			, "HUB_SRC"."SUPPLIER_BK" AS "SUPPLIER_BK"
			, "PIT_SRC2"."SNAPSHOT_TIMESTAMP" AS "SNAPSHOT__TIMESTAMP"
			, UPPER(MD5_HEX(  UPPER(REPLACE(COALESCE(TRIM( "HUB_SRC"."SUPPLIER_HKEY"),'~'),'^','\\' || '^'))||     )) AS "DIM_HASH_DIFF"
		FROM {{ ref('RDV_H_SUPPLIER') }} "HUB_SRC"
		INNER JOIN {{ source('BV', 'PIT_DETAIL_TRANS_SUPPLIER') }} "PIT_SRC2" ON  "HUB_SRC"."SUPPLIER_HKEY" = "PIT_SRC2"."SUPPLIER_HKEY"
		INNER JOIN {{ ref('RDV_S_SF1_SUPPLIER') }} "SAT_SRC2_1" ON  "PIT_SRC2"."S_SF1_SUPPLIER_HKEY" = "SAT_SRC2_1"."SUPPLIER_HKEY"
	)
	...
	SELECT 
		  UPPER(MD5_HEX(  || UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("FILTERED_DATA_SET"."SNAPSHOT__TIMESTAMP", 
			'DD/MM/YYYY HH24:MI:SS')),'~'),'^','\\' || '^'))|| '^'  )) AS "DIM_SUPPLIER_HKEY"
		, "FILTERED_DATA_SET"."BASE_OBJECT_H_KEY" AS "SUPPLIER_HKEY"
